...
// create: open(O_CREATE)、mkdir、symlink 等操作的公共入口。
// 步骤：
//   1) 获取父目录并加锁；
...
uint64 sys_open(void)
{
    ...
    // 仅当涉及写操作或创建时才包事务
    int need_tx = (omode & (O_CREATE | O_WRONLY | O_RDWR));
    if(need_tx)
        begin_transaction();
...
// sys_symlink: 创建符号链接，将目标路径字符串写入新 inode。
uint64 sys_symlink(void)
{
    ...
    int len = strlen(target) + 1;  // 存储目标路径并额外保存结尾的 '\0'
...
uint64 sys_exec(void)
{
    ...
    for(int i = 0; i < MAXARG; i++) {
        uint64 uarg;
        // 获取argv[i]的指针
        if(fetchaddr(uargv + i * sizeof(uint64), &uarg) < 0) {
            goto bad;
        }
        if(uarg == 0) {
            argv[i] = 0;
...
uint64 sys_mknod(void)
{
  ...
  argint(1, &major);
  argint(2, &minor);
  if((argstr(0, path, MAXPATH)) < 0 ||(ip = create(path, FD_DEVICE, major, minor)) == 0){
    end_transaction();
    return -1;
  }
  iunlockput(ip);
