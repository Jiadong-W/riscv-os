...
void timer_interrupt_handler(void)
{
    ...
    wakeup((void*)&ticks);
    release(&tickslock);
    scheduler_tick();        // 同步时间片消耗，必要时触发抢占
    ...
    if(scause_val & (1ULL << 63)) {
        ...
        if(interrupt_handlers[interrupt_code]) {
            ...
            if(interrupt_code == 5) {
                ...
                if(p && p->state == RUNNING && (p->exhausted_slice || p->preempt_pending)) {
                    yield();
                }
            }
        } else {
...
void usertrap(void)
{
    ...
    if(scause == 8) {
        ...
        if(irq == 5 && p->state == RUNNING && p->exhausted_slice)
            yield();
    } else {
        int handled = 0;
        if(scause == 15 || scause == 13){
            if(cow_resolve(p->pagetable, stval) == 0){
                handled = 1;
            }
        }
        if(!handled){
            // 无法通过写时复制修复的异常，按原逻辑终止进程
            user_handle_exception();
            p->killed = 1;
        }
    }
...
void user_handle_exception(void) {
    uint64 cause = r_scause();
    uint64 sepc = r_sepc();
    uint64 stval = r_stval();
    switch (cause) {
    case 2:
        printf("usertrap: 非法指令 sepc=0x%lx stval=0x%lx\n", sepc, stval);
        break;
    case 12:
        printf("usertrap: 指令页故障 sepc=0x%lx stval=0x%lx\n", sepc, stval);
        break;
    case 13:
        printf("usertrap: 加载页故障 sepc=0x%lx stval=0x%lx\n", sepc, stval);
        break;
    case 15:
        printf("usertrap: 存储页故障 sepc=0x%lx stval=0x%lx\n", sepc, stval);
        break;
    default:
        printf("usertrap: 未知异常 cause=%lu sepc=0x%lx stval=0x%lx\n", cause, sepc, stval);
        break;
    }
}
