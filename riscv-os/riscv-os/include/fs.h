...
// ======================== 文件系统核心描述 ========================
// 本文件给出伪磁盘的块布局、磁盘索引节点格式以及内存索引节点缓存结构。
...
#define MAX_FILE_BLOCKS (NDIRECT + NINDIRECT)
...
struct inode {
    ...
    short nlink;                  // 硬链接计数。
    uint32 size;                  // 文件当前字节长度。
    uint32 addrs[NDIRECT + 1];    // 数据块索引缓存，写回时同步到 dinode。
};
