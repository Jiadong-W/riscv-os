...
int uvmcopy(pagetable_t old, pagetable_t newp, uint64 sz)
{
    ...
    for(uint64 va = 0; va < sz_rounded; va += PGSIZE){
        ...
        uint64 pa = PTE2PA(*pte);
        uint perm = *pte & (PTE_R | PTE_W | PTE_X | PTE_U);
        // 为子进程分配一页物理内存，并拷贝父进程对应内容
        void *mem = alloc_page();
        if(mem == 0)
            goto err;

        memmove(mem, (void*)pa, PGSIZE);
        if(map_page(newp, va, (uint64)mem, perm) < 0){
            free_page(mem);
            goto err;
        }
        copied_pages++;
...
err:
        // 回滚已映射的页面，避免内存泄漏
    if(copied_pages > 0)
        uvmunmap(newp, 0, copied_pages, 1);
    return -1;
}
...
{
    if(sz > PGSIZE)
        panic("uvmfirst: init code too large");

    uint64 pa = (uint64)alloc_page();
    if(pa == 0)
        panic("uvmfirst: alloc_page");

    if(map_page(pagetable, 0, pa, PTE_R | PTE_W | PTE_X | PTE_U) < 0)
        panic("uvmfirst: map_page");

    memmove((void *)pa, src, sz);
}

// uvmalloc: 扩大用户地址空间 [oldsz, newsz)，逐页分配并清零。
uint64 uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
    ...
    for(uint64 a = start; a < end; a += PGSIZE) {
        ...
        memset(mem, 0, PGSIZE);
        if(map_page(pagetable, a, (uint64)mem, PTE_R | PTE_W | PTE_U) < 0) {
            free_page(mem);
            if(a > start)
                uvmdealloc(pagetable, a, oldsz);
            return 0;
